...
import {
  ...
  BookTitleInputSchema,
  BooksListInputSchema,
  BooksISBNListInputSchema,
} from '../schemas/books.js';
import {
  searchBooks,
  searchBooksCount,
 ...
 */
function registerSearchBooksToolTool(server: McpServer): void {
  server.registerTool(
    'search_books',
    {
      title: 'Search Books',
      ...
      try {
        ...
        if (params.response_format === 'json') {
          const output = {
            ...
            next_offset:
              total > params.offset + results.length
 ...
 */
function registerGetBookByISBNTool(server: McpServer): void {
  server.registerTool(
    'get_book_by_isbn',
    {
      ...
      try {
        ...
        if (params.response_format === 'json') {
          return {
            ...
            structuredContent: book as unknown as { [key: string]: unknown },
          };
        } else {
          const lines = [
            `# ${book.title}`,
 ...
 */
function registerGetBookByTitleTool(server: McpServer): void {
  server.registerTool(
    'get_book_by_title',
    {
 ...
 */
function registerGetBooksByTitlesTool(server: McpServer): void {
  server.registerTool(
    'get_books_by_titles',
    {
 ...
 */
function registerGetBooksByISBNListTool(server: McpServer): void {
  server.registerTool(
...
Returns only matching books. If no books found, an empty result is returned.`,
    ...
    async (params: BooksISBNListInput) => {
      try {
 ...
 */
function registerListBooksTool(server: McpServer): void {
  server.registerTool(
    'list_books',
    {
      ...
      try {
        ...
        if (params.response_format === 'json') {
          const output = {
            ...
            next_offset:
              total > params.offset + books.length
          ...
          if (total > params.offset + books.length) {
            lines.push(
              `_More results available. Use offset=${params.offset + params.limit} to fetch next page._`
            );
          }
