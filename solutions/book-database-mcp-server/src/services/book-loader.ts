...
let booksDetailsCache: BookDetail[] | null = null;
...
export async function loadBooks(): Promise<Book[]> {
  ...
  try {
    ...
    booksCache = JSON.parse(data) as Book[];
    return booksCache;
  } catch (error) {
    throw new Error(
      `Failed to load books: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
 ...
 */
export async function loadBookDetails(): Promise<BookDetail[]> {
  ...
  try {
    ...
    const parsed = JSON.parse(data) as { books: BookDetail[] };
    booksDetailsCache = parsed.books;
    return booksDetailsCache;
  } catch (error) {
    throw new Error(
      `Failed to load book details: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
 ...
 */
export async function getCombinedBooks(): Promise<BookRecord[]> {
 ...
 */
export async function searchBooksCount(query: string): Promise<number> {
  const books = await getCombinedBooks();
  const lowerQuery = query.toLowerCase();
 ...
 * Get book by ISBN
 */
export async function getBookByISBN(isbn: string): Promise<BookRecord | null> {
  const books = await getCombinedBooks();
 ...
 */
export async function getBooksByTitles(
  titles: string[]
): Promise<BookRecord[]> {
  const books = await getCombinedBooks();
 ...
 */
export async function getBooksByISBNList(
  isbns: string[]
): Promise<BookRecord[]> {
  const books = await getCombinedBooks();
